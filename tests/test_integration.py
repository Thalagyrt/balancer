            ...
            if node_name is None:
                ...
                def qemu(vmid=None):
                    if vmid is None:
                        ...
                        def get_vms():
                            migrated_to_this_node = [
                                call["vmid"]
                                for call in self.migration_calls
                                if call["opts"]["target"] == node_name
                            ]
                            migrated_from_this_node = [
                                call["vmid"]
                                for call in self.migration_calls
                                if call["opts"]["target"] != node_name
                            ]
                            ...
                            for r in self._resources_data:
                                ...
                                if (
                                    vmid in migrated_from_this_node
                                    and original_node == node_name
                                ):
...
class TestMigrateWorkloadNoMigration(unittest.TestCase):
    ...
    def test_no_migration_when_all_nodes_below_thresholds(self, mock_sleep):
        ...
        mock_api = MockProxmoxAPI(
            nodes, resources, backup_jobs, ha_rules, migration_lock_state="never"
        )
...
class TestMigrateWorkloadMemoryMigration(unittest.TestCase):
    ...
    def test_memory_migration_when_node_exceeds_memory_max(self, mock_sleep):
        ...
        mock_api = MockProxmoxAPI(
            nodes, resources, backup_jobs, ha_rules, migration_lock_state="success"
        )
...
class TestMigrateWorkloadMigrationFailure(unittest.TestCase):
    ...
    def test_migration_fails_when_lock_never_appears(self, mock_sleep):
        ...
        mock_api = MockProxmoxAPI(
            nodes, resources, backup_jobs, ha_rules, migration_lock_state="fail"
        )
